...
- 2026-03-04: начал и внедрил пакет №6 (mini cycle queue→sync→index→rank→memo): в автономном драйвере добавлен `run_fullspan_cycle()` для `run_fullspan_decision_cycle.py` (sync+build_run_index+strict/research rank), с guard на повторный запуск через `mini_cycle_state.txt` и триггером на момент перехода очереди в no-pending. Для таких очередей теперь автоматически генерируется decision-note с action=FULLSPAN_CYCLE и queue помечается как уже обработанная в текущем сеансе.
