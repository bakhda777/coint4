...
class GlobalRollingStatsManager:
    ...
    def initialize(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Use explicitly provided windows if available, otherwise determine from config
            if 'required_windows' in config and config['required_windows']:
                self.available_windows = set(config['required_windows'])
                logger.info(f"📊 Using explicitly provided windows: {sorted(self.available_windows)}")
            else:
                # Fallback to determining from config
                self.available_windows = determine_required_windows(config)
                logger.info(f"📊 Determined windows from config: {sorted(self.available_windows)}")
...
def get_global_rolling_manager() -> GlobalRollingStatsManager:
    ...
    if _global_manager is None:
        ...
        logger.info(f"🔄 Created new global rolling manager")
    else:
        logger.info(f"📊 Using existing global rolling manager (initialized={_global_manager.initialized})")
