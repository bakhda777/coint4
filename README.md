...
Leader holdout (next run, rollup leader): `docs/optimization_runs_20260116.md` with config `coint4/configs/best_config__leader_holdout_ssd25000__20260116_211943.yaml` (WFA 2024-05-01 → 2024-12-31, max_steps=5).
WFA queue watcher (CPU heartbeat, idle detection): `coint4/scripts/optimization/watch_wfa_queue.sh`.
WFA pair filtering: parallelized via `backtest.n_jobs` (default backend: threads; override with `COINT_FILTER_BACKEND=processes`, uses spawn for OpenMP safety) to maximize CPU usage.
