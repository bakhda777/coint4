        ...
        if results['success']:
            ...
            if trades:
                ...
                if not df.empty and 'final_pnl_r' in df.columns:
                    ...
                    # Step Risk Analysis needs Step-level data or parsing logs, 
                    # but we can approximate by checking if we see step breaches in logs (which user will see).
                    # Or we can check if any pair has huge cumulative loss.
                    # But since we don't have step grouping here easily without processing 'entry_time', 
                    # we'll rely on the tool output logs.
