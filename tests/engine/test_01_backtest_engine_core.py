...
def test_parameter_validation() -> None:
    ...
    try:
        BasePairBacktester(data, rolling_window=2, z_threshold=-1.0)
    ...
    try:
        BasePairBacktester(data, rolling_window=2, z_threshold=1.0, z_exit=1.5)
...
def test_empty_data_handling() -> None:
    ...
    bt_empty = BasePairBacktester(empty_data, rolling_window=3, z_threshold=1.0)
    # Should not raise exception during initialization
    
    # Test insufficient data
    small_data = pd.DataFrame({"Y": [1, 2], "X": [1, 2]})
    try:
        BasePairBacktester(small_data, rolling_window=5, z_threshold=1.0)
    ...
    bt = BasePairBacktester(min_data, rolling_window=3, z_threshold=1.0)
