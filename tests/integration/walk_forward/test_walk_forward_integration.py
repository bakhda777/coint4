...
class TestWalkForwardIntegration:
    ...
    def optimized_config(self):
        ...
        return {
            ...
            'backtest': {
                'rolling_window': 5 if os.environ.get('QUICK_TEST', '').lower() == 'true' else 10,  # ОПТИМИЗАЦИЯ
                'zscore_threshold': 2.0,
                'transaction_cost': 0.001,
                'market_regime_detection': False,  # Отключено для ускорения
                'hurst_window': 50 if os.environ.get('QUICK_TEST', '').lower() == 'true' else 100,  # ОПТИМИЗАЦИЯ
                'variance_ratio_window': 30 if os.environ.get('QUICK_TEST', '').lower() == 'true' else 50,  # ОПТИМИЗАЦИЯ
                'structural_break_protection': False,  # Отключено
                'cointegration_test_frequency': 50 if os.environ.get('QUICK_TEST', '').lower() == 'true' else 100,  # ОПТИМИЗАЦИЯ
                'adf_pvalue_threshold': 0.05,
                'exclusion_period_days': 1,  # Уменьшено
                'max_half_life_days': 5,  # Уменьшено
                'min_correlation_threshold': 0.3,
                'correlation_window': 30 if os.environ.get('QUICK_TEST', '').lower() == 'true' else 50,  # ОПТИМИЗАЦИЯ
                # Параметры оптимизации - упрощены
                'regime_check_frequency': 10,
                'use_market_regime_cache': True,
                'adf_check_frequency': 50 if os.environ.get('QUICK_TEST', '').lower() == 'true' else 100,  # ОПТИМИЗАЦИЯ,
    ...
    def test_memory_usage_optimization(self, sample_market_data, optimized_config):
        ...
        if os.environ.get('QUICK_TEST', '').lower() == 'true':
            pytest.skip("Пропускаем тяжелый тест памяти в режиме QUICK_TEST")
        import psutil
        import gc
    ...
    def test_error_handling_and_robustness(self, sample_market_data, optimized_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            corrupted_data.iloc[10:12, 0] = np.nan  # ОПТИМИЗАЦИЯ: меньший диапазон для малых данных
            ...
            extreme_config['backtest'].update({
                'regime_check_frequency': 100,  # ОПТИМИЗАЦИЯ: уменьшено для малых данных
                'adf_check_frequency': 500,     # ОПТИМИЗАЦИЯ: уменьшено для малых данных
