...
def optuna_test_config():
    ...
    return {
        ...
        'min_trials': MIN_TEST_TRIALS,
        'max_trials': MAX_TEST_TRIALS
    }
...
class TestConsolidatedOptuna:
    ...
    def _test_parameter_generation_without_log(self, study, config=None):
        ...
        # Проверяем минимальное количество trials (используем значения по умолчанию если config не передан)
        min_trials = config['min_trials'] if config else MIN_TEST_TRIALS
        max_trials = config['max_trials'] if config else MAX_TEST_TRIALS
