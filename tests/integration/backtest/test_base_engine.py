...
@pytest.mark.smoke
@pytest.mark.skip(reason="Эталонная реализация не соответствует текущей версии бэктестера")
...
@pytest.mark.unit
@pytest.mark.skip(reason="Take-profit логика была изменена, нужно обновить эталонную реализацию")
def test_take_profit_when_compared_to_reference_then_logic_correct(rng) -> None:
    ...
    data = pd.DataFrame({
        "Y": np.linspace(1, 15, N_SAMPLES) + rng.normal(0, NOISE_STD, size=N_SAMPLES),
        "X": np.linspace(1, 15, N_SAMPLES),
    })
...
def test_ols_cache_when_memory_limit_reached_then_controlled() -> None:
    ...
    # ОПТИМИЗАЦИЯ: Уменьшено с 5000 до 500 для ускорения
    import os
    N_PERIODS = 100 if os.environ.get('QUICK_TEST', '').lower() == 'true' else 500  # Было 5000
