...
class TestPairBacktesterIntegration:
    ...
    def test_pair_backtester_inherits_incremental_correctly(self):
        ...
        data = pd.DataFrame({
            'asset1': np.random.randn(20) + 100,
            'asset2': np.random.randn(20) + 50
        }, index=dates)
        
        backtester = PairBacktester(
            pair_name="TEST-INTEGRATION",
            pair_data=data.copy(),
            rolling_window=10,
            z_threshold=2.0,
    ...
    def test_run_on_day_incremental_behavior(self):
        ...
        x = np.cumsum(np.random.randn(30) * 0.1) + 100
        y = 2 * x + np.random.randn(30) * 0.5 + 50
        ...
        def mock_capital_calculation(risk_pct):
            ...
            base_capital = 1000 + call_count * 100
        ...
        for trade in open_trades:
            ...
            if matching_snapshot:
                ...
                assert abs(capital_used - expected_capital) < 0.01, \
                    f"Trade at {trade_date} used {capital_used}, expected {expected_capital}"
    
    def test_data_management_with_max_history_days(self):
    ...
    def test_portfolio_integration_with_real_calculations(self):
        ...
        # Create pair with realistic price movements
        np.random.seed(123)
