...
- Фильтрация пар: параллельная (n_jobs = `backtest.n_jobs`, backend `threads`; `COINT_FILTER_BACKEND=processes` + spawn для OpenMP‑безопасности) для полной загрузки CPU.
