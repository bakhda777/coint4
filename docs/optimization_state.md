...
Текущий снимок rollup: `2026-02-24 12:39:26Z` (из `coint4/artifacts/wfa/aggregate/rollup/run_index.md`).
...
Текущий снимок mini-rollup r07: `2026-02-24 14:41:23Z` (из `coint4/artifacts/wfa/aggregate/rollup_r07/run_index.md`).
...
### Candidate gate (prefilter: DD/tail/trades)

Единый prefilter-gate, который реально использует менеджер при выборе top-кандидатов из rollup: `tools/sprint_manager.py` (константа `CANDIDATE_GATE_V1`, override: `--candidate-*`):
- источник метрик: `coint4/artifacts/wfa/aggregate/rollup/run_index.json` (берём только `status=completed`; поля: `total_trades`, `max_drawdown_on_equity`, `tail_loss_worst_pair_pnl`, `tail_loss_worst_period_pnl`);
...
- ранжирование после gate: `Sharpe desc`, затем `|DD| asc`, затем `Trades desc`.
- fail-closed: если tail-метрик нет/они невалидны — кандидат отклоняется (override только для отладки: `--candidate-allow-missing-tail-metrics`).
- fail-closed (важно): если по gate **0 passed**, `tools/sprint_manager.py` не выбирает baseline/top-5 (пока не появятся валидные метрики в rollup).
...
- Ограничение просадки:
  ...
  - promote (fullspan v1): `worst_dd_pct <= 0.20`.
...
3. `cd coint4 && PYTHONPATH=src ./.venv/bin/python scripts/optimization/rank_multiwindow_robust_runs.py --run-index artifacts/wfa/aggregate/rollup/run_index.csv --contains <run_group_or_tag> --fullspan-policy-v1 --min-windows 1 --min-trades 200 --min-pairs 20 --max-dd-pct 0.20 --min-pnl 0 --min-coverage-ratio 0.95 --initial-capital 1000 --tail-quantile 0.20 --tail-q-soft-loss-pct 0.03 --tail-worst-soft-loss-pct 0.10 --tail-q-penalty 2.0 --tail-worst-penalty 1.0 --tail-worst-gate-pct 0.20`
...
- Этот порог не переопределяет fullspan hard-gate `worst_dd_pct <= 0.20`; для промоута действует только fullspan-контракт ниже.
...
- DD gate: `worst_dd_pct = max_window(max(abs(dd_holdout), abs(dd_stress))) <= 0.20`.
