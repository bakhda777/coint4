...
def build_universe(config_path: str = 'configs/universe.yaml', dry_run: bool = False):
    ...
    # Initialize data handler with explicit root if specified
    data_root = universe_cfg.get('data', {}).get('root')
    handler = DataHandler(app_cfg, root=data_root)
    
    # Set dates from config if available, otherwise compute
    if 'period' in universe_cfg:
        start_date = pd.Timestamp(universe_cfg['period']['start'])
        end_date = pd.Timestamp(universe_cfg['period']['end'])
    else:
    ...
    print(f"📁 Data root: {handler.data_dir}")
    
    # If dry-run, just print config and exit
    if dry_run:
        print("\n🏁 DRY RUN - Configuration verified:")
        print(f"  - Data root: {handler.data_dir}")
        print(f"  - Period: {start_date.date()} to {end_date.date()}")
        print(f"  - Timeframe: {universe_cfg['universe']['timeframe']}")
        print(f"  - Criteria: {universe_cfg['criteria']}")
        return None
...
def main():
    ...
    parser.add_argument('--dry-run', action='store_true',
                       help='Verify configuration without running')
    ...
    build_universe(args.config, dry_run=args.dry_run)
