#!/usr/bin/env python3
"""Diagnose holdout regression for 2026-01-19 relaxed4 configs."""
...
RUNS = {
    "holdout_corr0p45": RunSpec(
        run_id="holdout_relaxed4_20260119_pv0p08_hurst0p65_hl0p02_60_corr0p45_ssd50000_kpss0p03",
        run_dir=PROJECT_ROOT
        / "artifacts/wfa/runs/20260119_relaxed4_holdout/holdout_relaxed4_20260119_pv0p08_hurst0p65_hl0p02_60_corr0p45_ssd50000_kpss0p03",
    ),
    "holdout_corr0p5": RunSpec(
        run_id="holdout_relaxed4_20260119_pv0p08_hurst0p65_hl0p02_60_corr0p5_ssd50000_kpss0p03",
        run_dir=PROJECT_ROOT
        / "artifacts/wfa/runs/20260119_relaxed4_holdout/holdout_relaxed4_20260119_pv0p08_hurst0p65_hl0p02_60_corr0p5_ssd50000_kpss0p03",
    ),
    "wfa_corr0p45": RunSpec(
        run_id="stability_relaxed4_20260119_pv0p08_hurst0p65_hl0p02_60_corr0p45_ssd50000_kpss0p03",
        run_dir=PROJECT_ROOT
        / "artifacts/wfa/runs/20260119_stability_relaxed4/stability_relaxed4_20260119_pv0p08_hurst0p65_hl0p02_60_corr0p45_ssd50000_kpss0p03",
    ),
    "wfa_corr0p5": RunSpec(
        run_id="stability_relaxed4_20260119_pv0p08_hurst0p65_hl0p02_60_corr0p5_ssd50000_kpss0p03",
        run_dir=PROJECT_ROOT
        / "artifacts/wfa/runs/20260119_stability_relaxed4/stability_relaxed4_20260119_pv0p08_hurst0p65_hl0p02_60_corr0p5_ssd50000_kpss0p03",
    ),
}
...
def main() -> int:
    ...
    step_metrics.to_csv(
        RESULTS_DIR / "holdout_relaxed4_20260119_step_metrics.csv", index=False
    )

    pair_summary = pd.concat(pair_frames, ignore_index=True)
    pair_summary.to_csv(
        RESULTS_DIR / "holdout_relaxed4_20260119_pair_summary.csv", index=False
    )

    asset_summary = pd.concat(asset_frames, ignore_index=True)
    asset_summary.to_csv(
        RESULTS_DIR / "holdout_relaxed4_20260119_asset_summary.csv", index=False
    )

    concentration = pd.concat(concentration_frames, ignore_index=True)
    concentration.to_csv(
        RESULTS_DIR / "holdout_relaxed4_20260119_pair_concentration.csv", index=False
    )

    overlap = {
        "holdout_corr0p45": _overlap_summary(
            holdout_pairs=run_data["holdout_corr0p45"]["trade_stats"]["pair"].unique(),
            shortlist_pairs=run_data["wfa_corr0p45"]["trade_stats"]["pair"].unique(),
            holdout_pair_stats=pair_summary[
                pair_summary["run_id"] == RUNS["holdout_corr0p45"].run_id
            ],
        ),
        "holdout_corr0p5": _overlap_summary(
            holdout_pairs=run_data["holdout_corr0p5"]["trade_stats"]["pair"].unique(),
            shortlist_pairs=run_data["wfa_corr0p5"]["trade_stats"]["pair"].unique(),
            holdout_pair_stats=pair_summary[
                pair_summary["run_id"] == RUNS["holdout_corr0p5"].run_id
            ],
        ),
    }
    overlap_path = RESULTS_DIR / "holdout_relaxed4_20260119_overlap_summary.json"
    overlap_path.write_text(json.dumps(overlap, indent=2, sort_keys=True))

    filter_frames = []
    for key in ("holdout_corr0p45", "holdout_corr0p5", "wfa_corr0p45", "wfa_corr0p5"):
        spec = RUNS[key]
    ...
    filter_summary.to_csv(
        RESULTS_DIR / "holdout_relaxed4_20260119_filter_summary.csv", index=False
    )

    print("Step metrics:", RESULTS_DIR / "holdout_relaxed4_20260119_step_metrics.csv")
    print("Pair summary:", RESULTS_DIR / "holdout_relaxed4_20260119_pair_summary.csv")
    print("Asset summary:", RESULTS_DIR / "holdout_relaxed4_20260119_asset_summary.csv")
    print(
        "Pair concentration:",
        RESULTS_DIR / "holdout_relaxed4_20260119_pair_concentration.csv",
    )
    print("Overlap summary:", overlap_path)
    print("Filter summary:", RESULTS_DIR / "holdout_relaxed4_20260119_filter_summary.csv")
    return 0
