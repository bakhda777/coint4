...
for name in autonomous-wfa-driver.service autonomous-wfa-watchdog.service autonomous-wfa-watchdog.timer autonomous-queue-seeder.service autonomous-queue-seeder.timer autonomous-confirm-dispatch-agent.service autonomous-confirm-dispatch-agent.timer ops-sentinel-agent.service ops-sentinel-agent.timer; do
...
if [[ "$rc_reload" -ne 0 || "$rc_driver" -ne 0 || "$rc_timer" -ne 0 || "$rc_queue" -ne 0 || "$rc_confirm" -ne 0 || "$rc_sentinel" -ne 0 ]]; then
  echo "warning: failed to fully activate user systemd units (reload=$rc_reload driver=$rc_driver watchdog=$rc_timer queue_seeder=$rc_queue confirm_dispatch=$rc_confirm sentinel=$rc_sentinel)."
...
echo "autonomous WFA supervisor enabled (driver + watchdog + queue seeder timer + confirm dispatch timer + sentinel timer)."
