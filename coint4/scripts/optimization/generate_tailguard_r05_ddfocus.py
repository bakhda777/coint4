#!/usr/bin/env python3
"""Generate curated r05 dd-focus configs (tradeability+quality) around r04 v07 baseline.

Design goals:
- Do NOT touch risk/stop/z/dstop/maxpos axes (inherit from base v07).
- Run only on dd-focus window to speed up iterations (fast-loop).
- Vary only:
  H1 tradeability (meaningful thresholds not eaten by guardrail)
  H2 quality (moderate tighten; avoid collapsing to 1-2 pairs)
- Produce:
...
Run from app-root (coint4/):
  PYTHONPATH=src ./.venv/bin/python scripts/optimization/generate_tailguard_r05_ddfocus.py
"""
...
def _render_search_space_md(
    *,
    ...
    lines: list[str] = []
    lines.append("# tailguard_r05 dd-focus search space\n")
    lines.append(f"Date: {now}")
    lines.append(f"Run group: `{run_group}`")
    lines.append("Goal: fast-loop по max-DD участку; варьируем только tradeability+quality вокруг r04 v07.")
    lines.append("")
    ...
    lines.append("- Design: curated (no cartesian explosion)")
    lines.append("")
    lines.append("## Notes")
    lines.append("")
    lines.append("- Tradeability thresholds are set in real units (bid_ask_pct ~ 0.001 = 0.10%) so changes are NOT eaten by guardrail.")
    lines.append("- Risk/stop/z/dstop/maxpos axes are inherited from base and must remain unchanged.")
    lines.append("")
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    ap.add_argument(
        "--run-group",
        default="20260223_tailguard_r05_ddfocus",
    ...
    for variant in variants:
        ...
        holdout_name = f"holdout_tailguard_r05_ddfocus_{variant.variant_id}"
        ...
        stress_name = f"stress_tailguard_r05_ddfocus_{variant.variant_id}"
