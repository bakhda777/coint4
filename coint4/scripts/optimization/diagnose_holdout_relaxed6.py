#!/usr/bin/env python3
"""Diagnose holdout regression for 2026-01-19 relaxed6 configs."""
...
RUNS = {
    "holdout_w3m2": RunSpec(
        run_id="holdout_relaxed6_20260119_pv0p12_hurst0p7_hl0p02_60_corr0p45_ssd50000_kpss0p05_w3m2",
        run_dir=PROJECT_ROOT
        / "artifacts/wfa/runs/20260119_relaxed6_holdout/holdout_relaxed6_20260119_pv0p12_hurst0p7_hl0p02_60_corr0p45_ssd50000_kpss0p05_w3m2",
    ),
    "wfa_w3m2": RunSpec(
        run_id="stability_relaxed6_20260119_pv0p12_hurst0p7_hl0p02_60_corr0p45_ssd50000_kpss0p05_w3m2",
        run_dir=PROJECT_ROOT
        / "artifacts/wfa/runs/20260119_stability_relaxed6/stability_relaxed6_20260119_pv0p12_hurst0p7_hl0p02_60_corr0p45_ssd50000_kpss0p05_w3m2",
    ),
}
...
def main() -> int:
    ...
    step_metrics.to_csv(
        RESULTS_DIR / "holdout_relaxed6_20260119_step_metrics.csv", index=False
    )

    pair_summary = pd.concat(pair_frames, ignore_index=True)
    pair_summary.to_csv(
        RESULTS_DIR / "holdout_relaxed6_20260119_pair_summary.csv", index=False
    )

    asset_summary = pd.concat(asset_frames, ignore_index=True)
    asset_summary.to_csv(
        RESULTS_DIR / "holdout_relaxed6_20260119_asset_summary.csv", index=False
    )

    concentration = pd.concat(concentration_frames, ignore_index=True)
    concentration.to_csv(
        RESULTS_DIR / "holdout_relaxed6_20260119_pair_concentration.csv", index=False
    )
    ...
    overlap_path = RESULTS_DIR / "holdout_relaxed6_20260119_overlap_summary.json"
    ...
    filter_summary.to_csv(
        RESULTS_DIR / "holdout_relaxed6_20260119_filter_summary.csv", index=False
    )

    print("Step metrics:", RESULTS_DIR / "holdout_relaxed6_20260119_step_metrics.csv")
    print("Pair summary:", RESULTS_DIR / "holdout_relaxed6_20260119_pair_summary.csv")
    print("Asset summary:", RESULTS_DIR / "holdout_relaxed6_20260119_asset_summary.csv")
    print(
        "Pair concentration:",
        RESULTS_DIR / "holdout_relaxed6_20260119_pair_concentration.csv",
    )
    print("Overlap summary:", overlap_path)
    print("Filter summary:", RESULTS_DIR / "holdout_relaxed6_20260119_filter_summary.csv")
    return 0
