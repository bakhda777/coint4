#!/usr/bin/env python3
"""Generate r07 fullspan confirm configs for top dd-focus variants (r06-derived).

This is a *confirmation* sweep: small, curated (3 variants * holdout/stress),
intended to be run on the fullspan WFA period to validate whether dd-focus
promising knobs generalize.

Design goals:
- Keep the r04 v07 baseline as the base (risk/stop/z/dstop/maxpos unchanged).
- Only apply the tradeability/quality overrides corresponding to the selected
  r06 variants.
- Keep production universe: `configs/universe/pruned_v2_pairs_universe.yaml`.

Run from app-root (coint4/):
  PYTHONPATH=src ./.venv/bin/python scripts/optimization/generate_tailguard_r07_fullspan_confirm_top3.py
"""
...
def _variant_space() -> list[Variant]:
    # Selected to be representative (not duplicate configs) from r06 dd-focus diagnostics.
    return [
        Variant(
            "v01_from_r06v02_trade_mild_A",
            "Confirm: r06 v02 tradeability mild A on fullspan",
            {
        ...
        Variant(
            "v02_from_r06v03_trade_balanced_B",
            "Confirm: r06 v03 tradeability balanced B on fullspan",
            {
                "pair_selection.require_market_metrics": True,
                "pair_selection.require_same_quote": True,
        ...
        Variant(
            "v03_from_r06v05_bidask_only",
            "Confirm: r06 v05 bid-ask only tighten on fullspan",
            {"pair_selection.max_bid_ask_pct": 0.001},
        ),
    ]
...
def _render_search_space_md(*, run_group: str, base_path: Path, variants: list[Variant]) -> str:
    ...
    lines: list[str] = []
    lines.append("# tailguard_r07 fullspan confirm (top-3 from r06 dd-focus)\n")
...
def main() -> int:
    ...
    ap.add_argument(
        "--run-group",
        default="20260223_tailguard_r07_fullspan_confirm_top3",
    ...
    for variant in variants:
        ...
        holdout_name = f"holdout_tailguard_r07_fullspan_{variant.variant_id}"
        ...
        stress_name = f"stress_tailguard_r07_fullspan_{variant.variant_id}"
