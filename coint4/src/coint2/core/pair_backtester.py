...
class PairBacktester(IncrementalPairBacktester):
    ...
    def run_on_day(self, daily_data: pd.DataFrame, portfolio: Portfolio) -> float:
        ...
        # КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Передаем текущий индекс для предотвращения lookahead bias
        # Текущий индекс - это последняя строка в накопленных данных
        current_idx = len(self.pair_data)  # Используем длину как индекс последней строки
        
        # Process single period incrementally
        result = self.process_single_period(current_date, price_s1, price_s2, current_idx)
