...
Фильтрация пар WFA: параллелится через `backtest.n_jobs` (backend `threads` по умолчанию; `COINT_FILTER_BACKEND=processes` для override) для максимальной загрузки CPU.
