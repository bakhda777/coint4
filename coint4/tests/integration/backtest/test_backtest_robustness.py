...
DEFAULT_ROLLING_WINDOW = test_config['rolling_window']  # 20 в QUICK_TEST, иначе 30
...
class TestBacktestRobustness:
    ...
    def test_date_permutation_when_shuffled_then_destroys_performance(self):
        ...
        if os.environ.get('QUICK_TEST', '').lower() == 'true':
            pytest.skip("Пропускаем тест перемешивания дат в режиме QUICK_TEST")
    ...
    def test_time_shift_when_applied_then_degrades_performance(self):
        ...
        """
        if os.environ.get('QUICK_TEST', '').lower() == 'true':
            pytest.skip("Пропускаем тест смещения времени в режиме QUICK_TEST")
    ...
    def test_fee_sensitivity(self):
        if os.environ.get('QUICK_TEST', '').lower() == 'true':
            pytest.skip("Пропускаем тест чувствительности к комиссиям в режиме QUICK_TEST")
