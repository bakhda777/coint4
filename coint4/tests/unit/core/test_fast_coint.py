...
def test_fast_coint_when_compared_to_statsmodels_then_accurate(rng):
    ...
    N_SAMPLES = test_config['periods']  # 10 в QUICK_TEST, иначе 100
...
def test_fast_coint_performance_small(rng):
    ...
    test_config = get_test_config()
    N_SAMPLES = test_config['small_data_size']  # 50 в QUICK_TEST, иначе 200
    x = rng.normal(0, 1, N_SAMPLES).cumsum()
    y = rng.normal(0, 1, N_SAMPLES).cumsum()
...
def test_fast_coint_when_benchmarked_then_faster_than_statsmodels(rng):
    ...
    if os.environ.get('QUICK_TEST', '').lower() == 'true':
        pytest.skip("Пропускаем integration тест производительности в режиме QUICK_TEST")
    N_SAMPLES_LARGE = test_config['large_data_size']  # 200 в QUICK_TEST, иначе 1000
...
def test_fast_coint_when_pandas_series_then_compatible(rng):
    ...
    test_config = get_test_config()
    N_SAMPLES = test_config['small_data_size']  # 50 в QUICK_TEST, иначе 100
...
def test_fast_coint_when_nan_values_then_handled(rng):
    ...
    test_config = get_test_config()
    N_SAMPLES = test_config['data_size']  # 100 в QUICK_TEST, иначе 500
    x = rng.normal(0, 1, N_SAMPLES).cumsum()
    y = rng.normal(0, 1, N_SAMPLES).cumsum()
...
def test_fast_coint_when_edge_cases_then_handled_correctly(rng):
    ...
    test_config = get_test_config()
    SMALL_SAMPLE_SIZE = test_config['small_data_size']  # 50 в QUICK_TEST, иначе 100
