...
class TestOptimizationAcceleration:
    ...
    def test_global_rolling_cache_when_initialized_then_ready_for_use(self, rng):
        ...
        test_data = {}
        symbols = [f'SYM{i}' for i in range(min(test_config['n_symbols'], 4))]  # ОПТИМИЗАЦИЯ: динамическое количество символов
        dates = pd.date_range('2024-01-01', periods=test_config['periods'], freq='15min')  # ОПТИМИЗАЦИЯ: используем test_config
        ...
        with patch('src.coint2.core.memory_optimization.GLOBAL_PRICE', test_df):
            ...
            cache_config = {
                'search_space': {
                    'rolling_window': {'type': 'int', 'low': test_config['rolling_window'], 'high': test_config['rolling_window'] * 2}
                },
                'required_windows': [test_config['rolling_window']]  # ОПТИМИЗАЦИЯ: используем test_config
            }
    ...
    def test_cache_memory_efficiency_when_large_dataset_then_efficient(self, rng):
        ...
        test_config = get_test_config()
        symbols = [f'SYMBOL_{i:02d}' for i in range(min(test_config['n_symbols'] * 2, 20))]  # ОПТИМИЗАЦИЯ: адаптивное количество
        dates = pd.date_range('2024-01-01', periods=test_config['periods'], freq='15min')  # ОПТИМИЗАЦИЯ: используем test_config
        ...
        with patch('src.coint2.core.memory_optimization.GLOBAL_PRICE', test_df):
            cache_config = {
                'search_space': {
                    'rolling_window': {'type': 'int', 'low': test_config['rolling_window'], 'high': test_config['rolling_window'] * 3}
                },
                'required_windows': [test_config['rolling_window'], test_config['rolling_window'] * 2]  # ОПТИМИЗАЦИЯ: адаптивные окна
            }
